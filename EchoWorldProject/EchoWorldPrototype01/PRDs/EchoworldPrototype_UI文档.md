...
### 1.0 Bug修复与功能更新（2026-03-24）

**修复的问题：**
- ✅ 所有页面添加 `SafeAreaView` 适配，确保内容不被状态栏遮挡
- ✅ 返回按钮尺寸从 `44x44pt` 扩大到 `56x56pt`，提升触摸体验
- ✅ 删除聊天界面输入框左侧的礼物图标（🎁）
- ✅ 修复角色创建流程中成熟度选择的验证逻辑，必须选择选项才能进入下一步
- ✅ 修复帖子详情页不能正常显示的问题（通过保存生成的帖子数据到mockApi）

**新增功能：**
- ✅ Timeline 页面添加测试数据（为每个角色生成5条随机动态）
- ✅ World 页面添加5个景点标记，支持点击查看详情
- ✅ 支持多角色创建和管理
- ✅ Timeline 支持按角色筛选功能
- ✅ 设置页面显示所有角色信息，添加清空数据功能
- ✅ 登录状态和角色数据持久化存储（AsyncStorage）
